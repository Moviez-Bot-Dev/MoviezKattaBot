...
async def ddl_call_back(client, query):
    ...
    try:
        if "youtu" in youtube_dl_url or "youtube" in youtube_dl_url:
            ...
            xMANIA_BAAPUx_d_size = requests.head(youtube_dl_url)    
            xMANIA_BAAPUx_t_length = int(xMANIA_BAAPUx_d_size.headers.get("Content-Length", 0))
            xMANIA_BAAPUx_path = urlparse(youtube_dl_url).path
            xMANIA_BAAPUx_u_name = os.path.basename(xMANIA_BAAPUx_path)
            total_length = humanbytes(xMANIA_BAAPUx_t_length)
        logger.info(total_length)
        sizee = "undefined" if "youtu" in youtube_dl_url or "youtube" in youtube_dl_url else total_length
        namee = "undefined" if "youtu" in youtube_dl_url or "youtube" in youtube_dl_url else xMANIA_BAAPUx_u_name
    except Exception as e:
        logger.error(f"Something went wrong in the code =>::: {e}")
    ######################## 
    try:
        mania_sticker = await query.message.reply_sticker(sticker=random.choice(maniastickerset))
    except Exception as e:
        await print(e)
        pass
    ...
    if os.path.exists(download_directory):
        ...
        if file_size > TG_MAX_FILE_SIZE:
            ...
            if (await db.get_upload_as_doc(query.from_user.id)) is False:
                ...
                await mania_sticker.delete()
                try:
                    mania_sticker01 = await query.message.reply_sticker(sticker=random.choice(maniastickerset))
                except Exception as e:
                ...
                await client.send_document(
                    ...
                    progress_args=(
                        script.MANIA_UPLOAD_START.format(custom_file_name),
                        query.message,
                        start_time
                    )
                )
                await mania_sticker01.delete()
            else:
                 width, height, duration = await Mdata01(download_directory)
                 thumb_image_path = await Gthumb02(client, query, duration, download_directory)
                 await mania_sticker.delete()
                 try:
                     mania_sticker02 = await query.message.reply_sticker(sticker=random.choice(maniastickerset))
                 except Exception as e:
                     await client.send_message(chat_id = query.message.chat.id, text=f"🥳")
                     pass
                 ...
                 await mania_sticker02.delete()
            if tg_send_type == "audio":
                duration = await Mdata03(download_directory)
                thumbnail = await Gthumb01(client, query)
                await mania_sticker.delete()
                try:
                    mania_sticker03 = await query.message.reply_sticker(sticker=random.choice(maniastickerset))
                except Exception as e:
                    await client.send_message(chat_id = query.message.chat.id, text=f"🥳")
                    pass
                ...
                await mania_sticker03.delete()
            elif tg_send_type == "vm":
                width, duration = await Mdata02(download_directory)
                thumbnail = await Gthumb02(client, query, duration, download_directory)
                await mania_sticker.delete()
                try:
                    mania_sticker04 = await query.message.reply_sticker(sticker=random.choice(maniastickerset))
                except Exception as e:
                    await client.send_message(chat_id = query.message.chat.id, text=f"🥳")
                    pass
...
async def download_coroutine(bot, session, custom_file_name, url, file_name, chat_id, message_id, start):
    ...
    async with session.get(url, timeout=PROCESS_MAX_TIMEOUT) as response:
        ...
        xMANIA_BAAPUx_path = urlparse(url).path
        xMANIA_BAAPUx_u_name = os.path.basename(xMANIA_BAAPUx_path)
        ...
        await bot.edit_message_text(
            ...
            """.format(xMANIA_BAAPUx_u_name, humanbytes(total_length))
        )
        with open(file_name, "wb") as f_handle:
            while True:
                ...
                if round(diff % 5.00) == 0 or downloaded == total_length:
                    ...
                    xxMANIA_BAPUXX_total_size = humanbytes(total_length)
                    tp = round(percentage, 2)
                    xxMANIA_BAPUXX_estimated_total_time = TimeFormatter(milliseconds=estimated_total_time)
                    template_name = custom_file_name if custom_file_name else "**⚠ You haven't given any custom name...**"

                    xLDx = f"**ღ♡ ʀᴜɴɴɪɴɢ ᴍᴀɴɪᴀ ᴄᴏɴꜱᴛʀᴜᴄᴛɪᴏɴ ♡♪**\n**ᵉⁿʲᵒʸ ˢᵘᵖᵉʳᶠᵃˢᵗ ᵈᵒʷⁿˡᵒᵈ ᵇʸ [ᴍᴀɴɪᴀ](https://t.me/Mania24SupportBot)◔_◔** \n\n**░░✩ 📂𝐎𝐑𝐆 𝐅𝐈𝐋𝐄𝐍𝐀𝐌𝐄 ✩ **\n<code>{xMANIA_BAAPUx_u_name}</code>\n\n**░░✩ 📝𝐍𝐄𝐖 𝐍𝐀𝐌𝐄 ✩ **\n<code>{template_name}</code>\n\n ☼﹍︿﹍ⲯ﹍︿﹍﹍︿﹍ⲯ﹍︿﹍☼\n⚡️**Done:{tp}**%| 🧬ѕιzє: {xxMANIA_BAPUXX_total_size}"
                    progress = "{0}{1}".format(
                    ...
                    tmp = xLDx + "\n" + progress + script.PROGRESS_BAR.format( 
                        ...
                        humanbytes(speed),
                        xxMANIA_BAPUXX_estimated_total_time if xxMANIA_BAPUXX_estimated_total_time != '' else "0 s"
                    )
                    try:
                        current_message = tmp
