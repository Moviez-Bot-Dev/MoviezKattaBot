...
from database.mania_ffmpeg import take_screen_shot
from info import DOWNLOAD_LOCATION, AUTH_CHANNEL
from database.users_chats_db import db
from plugins.settings.settings import *
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery, ForceReply
from maniabot.forcesub import handle_force_subscribe
...
async def addthumbs(client, message):
    ...
    ManiaDev = await message.reply_text("Please Wait ...")
    ...
    if replied and replied.photo:
        ...
        await ManiaDev.edit("**✅ Custom thumbnail set successfully!**")
    else:
...
async def viewthumbnail(client, message):    
    ...
    if AUTH_CHANNEL:
      ...
      if fsub == 400:
        return   
    thumbnail = await db.get_mania_thumbnail(message.from_user.id)
...
async def removethumbnail(client, message):
    ...
    await db.set_mania_thumbnail(message.from_user.id, thumbnail=None)
...
@Client.on_message(filters.private & filters.command(['set_mania_thumb','set_mania_thumbnail', 'smt']))
async def add_thumbnail(client, message):
    ...
    if replied and replied.photo:
        # Save the photo file_id as a thumbnail for the user
        await db.set_mania_thumbnail(message.from_user.id, thumbnail=replied.photo.file_id)
...
async def Gthumb01(bot, update):
    ...
    db_thumbnail = await db.get_mania_thumbnail(update.from_user.id)
