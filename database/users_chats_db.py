...
class Database:
    ...
    async def get_banned(self):
        ...
        is_verified = self.grp.find({'chat_status.is_mania_verified': True})
        b_chats = [chat['id'] async for chat in chats]
        b_users = [user['id'] async for user in users]
        mz_verified = [chat['id'] async for chat in is_verified]
        return b_users, b_chats, mz_verified
  
    async def verify_mania_chat(self, chat):
        chat_status=dict(
            is_mania_verified=True,
            )
    ...
    async def set_mania_thumbnail(self, id, thumbnail):
        await self.col.update_one({'id': id}, {'$set': {'thumbnail': thumbnail}})

    async def get_mania_thumbnail(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('thumbnail', None)

    async def get_mania_caption(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('mania_caption', None)

    async def get_user_data(self, id) -> dict:
        user = await self.col.find_one({'id': int(id)})
        return user or None
        
    # Thank you @Mania24SupportBot 
