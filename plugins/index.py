...
from info import ADMINS, MANIA_RENAMERS
from info import INDEX_REQ_CHANNEL as LOG_CHANNEL
from info import MANIA_MODE 
...
from maniabot import MoviezMateBot 
...
async def send_for_index(bot, message):
    ...
    if message.from_user.id in ADMINS:
        if (MANIA_MODE==True):
            ...
            buttons = [
                [ InlineKeyboardButton("📝✧ sᴛᴀʀᴛ ʀᴇɴᴀᴍɪɴɢ ✧📝", callback_data="rename") ],
                [ InlineKeyboardButton('📇✧ sᴛᴀʀᴛ ɪɴᴅᴇxɪɴɢ ✧📇',callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}')],
            ...
            buttons = [
                ...
                [
                    InlineKeyboardButton('⨳  Ꮯ Ꮮ ϴ Տ Ꭼ  ⨳', callback_data='close_data'),
                ]
            ]
            reply_markup = InlineKeyboardMarkup(buttons)
            return await message.reply(
    ...
    await bot.send_message(LOG_CHANNEL,
                           ...
                           reply_markup=reply_markup)
    if (MANIA_MODE == True):
        if message.from_user.id in MANIA_RENAMERS:
            ...
            await message.reply('🎉\n\n\n❤️ Thank You For the Contribution, Wait For My Moderators to verify the files.\n\n\n🎁')
            buttons = [
                        [InlineKeyboardButton("📝✧ sᴛᴀʀᴛ ʀᴇɴᴀᴍɪɴɢ ✧📝", callback_data="requireauth") ],
                        [InlineKeyboardButton('⨳  Ꮯ Ꮮ ϴ Տ Ꭼ  ⨳', callback_data='cancel')]]
            reply_markup = InlineKeyboardMarkup(buttons)
            file = getattr(message, message.media.value)
            filename = file.file_name
            filesize = humanize.naturalsize(file.file_size) 
            k = await message.reply(
                                f"\n⨳ *•.¸♡ ᴍᴀɴɪᴀ ᴍᴏᴅᴇ ♡¸.•* ⨳\n\n🤩 Do you know MoviezMate can do a lot of things at a time...\nWould you like to try some of it's amazing features... \n\n🪬Chat ID/ Username: <code>{chat_id}</code>\nℹ️Last Message ID: <code>{last_msg_id}</code> \n\n🎞**File Name** :- `{filename}`\n\n⚙️**File Size** :- `{filesize}`",
                                reply_to_message_id=message.id,
                                reply_markup=reply_markup)
            await asyncio.sleep(600)
            await k.delete()
